...
int getPass(const std::string& vaultName, std::vector<uint8_t>* key) {
    ...
    // Zero out decrypted memory after listing
    sodium_memzero(decrypted.data(), decrypted.size());

    // Get user selection
    std::string choiceStr;
    std::cout << "Select entry (name or number): ";
    ...
    const auto& entry = vJson["entries"][entryIndex];
    std::string entryName = entry["name"].get<std::string>();
    std::string entryPassword = entry["password"].get<std::string>();
    std::string entryComments = entry["comments"].get<std::string>();

    // Display entry details
    std::cout << "\n=== Entry Details ===\n";
    std::cout << "Name:     " << entryName << "\n";
    std::cout << "Password: " << entryPassword << "\n";
    if (!entryComments.empty()) {
        std::cout << "Comments: " << entryComments << "\n";
    }
    std::cout << "\n";

    // Update lastUsed timestamp
    vJson["entries"][entryIndex]["lastUsed"] = getCurrentTimestamp();
